...
function BankReconciliationReport({loginData}) {
    ...
    const ReportColumns = [
        ...
        { "field": "debit_account", "headerName": "Debit Account", "width":300 },
        { "field": "credit_account", "headerName": "Credit Account", "width": 300 },
        ...
        if (report_data.start_date !== '' && report_data.end_date !== '') {
            ...
            if (transaction_list.length > 0) {
                ...
                rows.push({
                    ...
                    "debit_account": '--',
                    "credit_account": '--',
                    "amount": currencyConverter(sumObjectArray(transaction_list, 'amount')),
                    "inserted_by": '--',
                    "inserted_date": '--'
                })
    ...
    useEffect(() => {
        if (login.permission.filter(e=>e.permission === 'finance_report_bank_reconciliation').length < 1) {
            navigate('/')
        }
        getData().then((r) => {});
    }, []);
                    ...
                    <>
                        <div className="row">
                            <div className="col-md-9 col-10">
                                <Breadcrumbs
                                    title='Bank Reconciliation Report' data={[{title: 'Finance'}, {title: 'Report'}, {title: 'Bank Reconciliation'}]}
                                />
                            </div>
                        </div>
...
export default connect(mapStateToProps, null)(BankReconciliationReport)