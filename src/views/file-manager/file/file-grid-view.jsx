...
function FileGridView(props) {
    ...
    const getData = async () => {
        await axios.get(`${serverLink}file-manager/file/list/${atob(slug)}/${user_id}`, token)
            .then((result) => {
    ...
    return isLoading ? <SpinnerLoader/> : (<>
            <tr>
                <td>
                    <a href={`${serverLink}public/upload/files/${formData.file_path}`}  onClick={async ()=>{ trackViewedFile(formData); }}>
                        {getItemIcon(formData.file_type)}
                        <span className='align-middle fw-bold'> {formData.file_name}</span>
                    </a>
                </td>
                <td>{formData.file_type}</td>
                <td>
                    {formData.file_size} MB
                </td>
                <td>
                ...
                <td>
                    <UncontrolledDropdown>
                        ...
                        <DropdownMenu>
                            <a href={`${serverLink}public/upload/files/${formData.file_path}`}  onClick={async ()=>{ trackViewedFile(formData); }}><DropdownItem className='w-100'><Eye className='me-50' size={15} /> <span className='align-middle'>View</span></DropdownItem></a>
                            <DropdownItem  className='w-100' onClick={() => {
                                setFormData({
                                    ...formData,
...
export default connect(mapStateToProps, mapDispatchToProps)(FileGridView)