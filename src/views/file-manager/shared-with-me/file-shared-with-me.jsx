...
import {CheckSquare, Eye, FileText, Layers, MoreVertical, Trash} from "react-feather";
import {Fragment, useEffect, useState} from "react";
import {formatDateAndTime, generate_token, serverLink, showIcon} from "@src/resources/constants";
...
function FileSharedWithMe(props) {
    ...
    const getData = async () => {
        await axios.get(`${serverLink}file-manager/file/shared-with-me/list/${atob(slug)}/${user_id}`, token)
            .then((result) => {
                ...
                action:  (
                    <div className="d-flex">
                        <UncontrolledDropdown>
                            ...
                            <DropdownMenu>
                                <a href={`${serverLink}public/upload/files/${item.file_path}`}  onClick={async ()=>{ trackViewedFile(item.file_id); }}><DropdownItem className='w-100'><Eye className='me-50' size={15} /> <span className='align-middle'>View</span></DropdownItem></a>
                                ...
                                <DropdownItem className='w-100'  onClick={() => {
                                    showConfirm("Warning", `Are you sure you want to exit from this file?`, "warning")
                                        .then( async (confirm) => {
                                            if (confirm) {
                                                onDelete(item)                                        }
                                        })
                                }}>
                                    <Trash className='me-50' size={15} /> <span className='align-middle'>Exit from file</span>
                                </DropdownItem>
                            </DropdownMenu>
                        </UncontrolledDropdown>
                    </div>
    ...
    const onDelete = async (item) => {
        ...
        await axios.post(`${serverLink}file-manager/file/shared-with-me/delete`, sendData, token)
            .then(data => {
    ...
    const trackViewedFolder = async () => {
        await axios.patch(`${serverLink}file-manager/folder/viewed`, formData, token)
            .then(() => {}).catch(() => {toast.error("NETWORK ERROR");});
    };

    const trackViewedFile = async (id) => {
        const sendData = {
            ...formData,
            file_id: id,
        };
                        ...
                        <div className="row">
                            <div className="col-md-9 col-8">
                                <Breadcrumbs title='Shared with me'
                                             data={[{title: 'Shared with me'}, {title: 'Files'}]}/>
                            </div>

                        </div>
                        <div className='blog-wrapper'>
                            <DocumentData/>
                            <MiddleModal title={"Upload File Form"} size={'modal-lg'} open={modalOpen} toggleSidebar={toggleSidebar}>
                                <FileForm
                                    onEdit={onEdit}
                                    onSubmit={onSubmit}
                            ...
                            <MiddleModal title={"Action Form"} size={'modal-lg'} open={modalOpen3} toggleSidebar={toggleSidebar3}>
                                <FileAction
                            ...
                            <MiddleModal title={"Edit File Name"} size={'modal-lg'} open={modalOpen4} toggleSidebar={toggleSidebar4}>
                                <EditFileName
                            ...
                            <MiddleModal title={"Share Link Generated"} size={'modal-lg'} open={modalOpen5} toggleSidebar={toggleSidebar5}>
                                <FileShare
                                    onSubmit={onShare}
                            ...
                            <MiddleModal title={formData.file_name} size={'modal-lg'} open={modalOpen2} toggleSidebar={toggleSidebar2}>
                                <FileTrack
                            ...
                            <MiddleModal title={formData.file_name} subtitle={'Tracking Details'} size={'modal-lg'} open={modalOpen6} toggleSidebar={toggleSidebar6}>
                                <FileTrackingDetails
...
export default connect(mapStateToProps, mapDispatchToProps)(FileSharedWithMe)