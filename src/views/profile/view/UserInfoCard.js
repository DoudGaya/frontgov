...
import {formatDateAndTime, generate_token, serverLink} from "@src/resources/constants";
...
const UserInfoCard = ({ profile, reload, setRebuildDashboard, token, loginData }) => {
  ...
  const renderUserSignature = () => {
    if (profile.length > 0 && typeof data.signature !== null) {
      return (
        <img crossOrigin="anonymous" height='50' width='50' alt={data.full_name} src={`${serverLink}public/upload/user/${data.signature}`} className='img-fluid rounded mt-3 mb-2'/>
      )
    }
  }
