...
function Budget({loginData}) {
    ...
    const initialValue = {
        budget_id: "", order_date: "", budget_code: generate_token(10), approved_amount: 0, created_by: loginData[0]?.user_id, agency_id: login.agency_id,
    ...
    const getData = async () => {
        await axios.get(`${serverLink}budget/my_budget/${login.user_id}`, token)
            .then((result) => {
    ...
    useEffect(() => {
        if (login.permission.filter(e=>e.permission === 'budget_my_budget').length < 1) {
            navigate('/')
        }
        getData().then(r => {});
    }, []);
                    ...
                    <>
                        <div className="row">
                            <div className="col-md-9 col-10">
                                <Breadcrumbs
                                    title='My Budget' data={[{title: 'Budget'}, {title: 'My Budget'}]}
                                />
                            </div>
